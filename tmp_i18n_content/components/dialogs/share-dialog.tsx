          ...
          {currentVisibility === 'private' ? (
            <div className="space-y-3">
              <div className="flex items-center gap-3 p-3 bg-muted/50 rounded-lg border">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium">{t('share.status.private')}</p>
                  <p className="text-xs text-muted-foreground">{t('share.status.privateInfo')}</p>
                </div>
              </div>
                ...
                <Button onClick={handleMakePublic} disabled={isChangingVisibility} className="w-full">
                  {isChangingVisibility ? (
                    <>
                      ...
                      {t('share.actions.makingPublic')}
                    </>
                  ) : (
                    <>
                      <GlobeHemisphereWestIcon size={16} className="mr-2" />
                      {t('share.actions.makePublic')}
                    </>
                  )}
                </Button>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 p-3 bg-blue-50 dark:bg-blue-950/50 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-blue-900 dark:text-blue-100">{t('share.status.public')}</p>
                  <p className="text-xs text-blue-700 dark:text-blue-300">{t('share.status.publicInfo')}</p>
                </div>
                <Button
                  variant="outline"
                  size="sm"
                ...
                >
                  ...
                  {isChangingVisibility ? t('share.actions.makingPrivate') : t('share.actions.makePrivate')}
                </Button>
              </div>

              {/* CopyIcon Link */}
              <div className="space-y-2">
                <label className="text-sm font-medium">{t('share.link.label')}</label>
