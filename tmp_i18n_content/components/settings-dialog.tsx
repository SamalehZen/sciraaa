...
import { swapLocale, getLocaleFromPath } from '@/lib/i18n/path';
...
function PreferencesSection({
  ...
  const initialLocale = (readCookie('locale') as 'fr' | 'en') || getLocaleFromPath(pathname || '/') || 'fr';
  const [language, setLanguage] = useState<'fr' | 'en'>(initialLocale);
  const handleLanguageChange = (value: 'fr' | 'en') => {
    setLanguage(value);
    const maxAge = 60 * 60 * 24 * 30;
    document.cookie = `locale=${value}; Max-Age=${maxAge}; Path=/; SameSite=Lax`;
    const newPath = swapLocale(window.location.pathname + window.location.search, value);
    ...
    <div className={cn('space-y-6', isMobile ? 'space-y-4' : 'space-y-6')}>
      ...
      <div className="space-y-3">
        <div className="space-y-2.5">
          ...
          <div className="space-y-2.5">
            <Select value={language} onValueChange={(v) => handleLanguageChange(v as 'fr' | 'en')}>
