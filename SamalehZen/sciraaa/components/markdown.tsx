...
const LazyCodeBlockComponent: React.FC<CodeBlockProps> = ({ children, language, elementKey }) => {
  ...
  const handleCopy = useCallback(async () => {
    try {
      ...
      toast.error(t('general.copy.failed'));
    }
  }, [children]);
  ...
  const toggleWrap = useCallback(() => {
    setIsWrapped((prev) => {
      const newState = !prev;
      toast.success(newState ? t('markdown.wrap.enabled') : t('markdown.wrap.disabled'));
      return newState;
    });
  }, []);
    ...
    <div className="group relative my-5 rounded-md border border-border bg-accent overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2 bg-accent border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground">{t('markdown.lines', { count: lineCount })}</span>
        </div>
        ...
        <div className="flex gap-1">
          <button
            ...
            title={isWrapped ? t('markdown.wrap.disable') : t('markdown.wrap.enable')}
          >
          ...
          <button
            ...
            title={isCopied ? t('markdown.copied') : t('markdown.copyCode')}
          >
...
const InlineCode: React.FC<{ code: string; elementKey: string }> = React.memo(({ code }) => {
  ...
  return (
    <code
      ...
      onClick={handleCopy}
      title={isCopied ? t('markdown.copied') : t('markdown.clickToCopy')}
    >
      {code}
    </code>
  );
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <Button
              ...
              aria-label={t('markdown.downloadCsv')}
            >
