  ...
  ({
    ...
    useEffect(() => {
      ...
      if (currentModelExists && currentModelRequiresPro && !isProUser && selectedModel !== 'scira-default') {
        ...
        toast.info(t('search.models.switchedDefault'));
      }
    ...
    const renderModelCommandContent = () => (
      ...
      >
        {!isMobile && (
          <CommandInput
            placeholder={t('search.models.placeholder')}
            className="h-9"
            value={searchQuery}
...
const GroupModeToggle: React.FC<GroupSelectorProps> = React.memo(
  ({ selectedGroup, onGroupSelect, status, onOpenSettings, isProUser }) => {
    ...
    const GroupSelectionContent = () => (
      ...
      >
        <CommandInput placeholder={t('search.modes.placeholder')} className="h-9" />
        <CommandEmpty>{t('search.modes.empty')}</CommandEmpty>
            ...
            <Drawer open={open} onOpenChange={handleOpenChange}>
              ...
              <DrawerContent className="max-h-[80vh]">
                <DrawerHeader className="text-left pb-4">
                  <DrawerTitle>{t('search.modes.chooseTitle')}</DrawerTitle>
                </DrawerHeader>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center gap-1.5">
                  ...
                  <p className="font-semibold text-xs">
                    {isExtreme
                      ? t('search.extreme.active')
                      : session
                        ? t('search.extreme.label')
                        : t('search.extreme.signInRequired')
                    }
                  </p>
                </div>
                <p className="text-[11px] leading-snug text-secondary">
                  {t('search.extreme.description')}
                </p>
                {!isProUser && (
                  <div className="pt-1 border-t border-border/50">
                    <a
            ...
            >
              ...
              {isRecording ? (
                ...
                <Textarea
                  ...
                  placeholder={
                    isEnhancing
                      ? t('search.input.enhancing')
                      : isTypewriting
                        ? t('search.input.writing')
                        : hasInteracted
                          ? t('search.input.askNew')
                          : t('search.input.askDefault')
                  }
                  value={input}
                ...
                <div className={cn('flex items-center gap-2')}>
                  ...
                  <ModelSwitcher
                    ...
                    onModelSelect={(model) => {
                      ...
                      toast.message(t('search.models.switchedTo', { label: model.label }), {
                        description: isVisionModel ? t('search.models.visionEnabled') : undefined,
                    ...
                    <Tooltip delayDuration={300}>
                      ...
                      >
                        <div className="flex flex-col gap-0.5">
                          <span className="font-medium text-[11px]">{t('search.attach.title')}</span>
                          <span className="text-[10px] text-accent leading-tight">
                            {hasPdfSupport(selectedModel) ? t('search.attach.subtitlePdf') : t('search.attach.subtitleImage')}
                          </span>
                        </div>
                      </TooltipContent>
                    </Tooltip>
                      ...
                      >
                        <div className="flex flex-col gap-0.5">
                          <span className="font-medium text-[11px]">
                            {isEnhancing ? t('search.enhance.titleEnhancing') : isTypewriting ? t('search.enhance.titleWriting') : t('search.enhance.titleDefault')}
                          </span>
                          <span className="text-[10px] text-accent leading-tight">
                            {isEnhancing
                              ? t('search.enhance.descImproving')
                              : isTypewriting
                                ? t('search.enhance.descTyping')
                                : isProUser
                                  ? t('search.enhance.descDefault')
                                  : t('search.enhance.descPro')}
                          </span>
                        </div>
                      </TooltipContent>
                    </Tooltip>
                  ...
                  {isProcessing ? (
                    <Tooltip delayDuration={300}>
                      ...
                      <TooltipContent
                        ...
                        className="border-0 backdrop-blur-xs py-2 px-3 !shadow-none"
                      >
                        <span className="font-medium text-[11px]">{t('search.stopGeneration')}</span>
                      </TooltipContent>
                    </Tooltip>
                    ...
                    <Tooltip delayDuration={300}>
                      ...
                      >
                        <div className="flex flex-col gap-0.5">
                          <span className="font-medium text-[11px]">
                            {isRecording ? t('search.voice.stop') : t('search.voice.input')}
                          </span>
                          <span className="text-[10px] text-accent leading-tight">
                            {isRecording ? t('search.voice.clickStop') : t('search.voice.record')}
                          </span>
                        </div>
                      </TooltipContent>
                    </Tooltip>
                    ...
                    <Tooltip delayDuration={300}>
                      ...
                      >
                        <div className="text-center">
                          <div className="font-medium text-[11px] mb-1">{t('search.send.title')}</div>
                          <div className="text-[10px] text-accent space-y-0.5">
                            <div className="flex items-center justify-center gap-1.5">
                              ...
                              <span>{t('search.send.hint.newLine')}</span>
                            </div>
                            <div className="flex items-center justify-center gap-1.5">
                              <Kbd className="rounded text-[9px] px-0.5 h-4 min-w-4">Enter</Kbd>
                              <span>{t('search.send.hint.send')}</span>
                            </div>
                          </div>
                        </div>
                      </TooltipContent>
