...
import { ProBadge } from './form/ProBadge';
import { ModelSwitcher } from './form/ModelSwitcher';
import { CONNECTOR_CONFIGS, CONNECTOR_ICONS } from './form/ConnectorConfigs';
import {
  DiscountConfig,
  ConnectorProvider,
  ConnectorConfig,
...
const GroupModeToggle: React.FC<GroupSelectorProps> = React.memo(
  ({ selectedGroup, onGroupSelect, status, onOpenSettings, isProUser }) => {
    ...
    const [open, setOpen] = useState(false);
    const isMobile = useIsMobile();
    const pdfSpecialGroup: SearchGroupId = 'pdfExcel';
    const fallbackGroup: SearchGroupId = 'libeller';
    const isPdfMode = selectedGroup === pdfSpecialGroup;
    ...
    const visibleGroups = useMemo(
      () =>
        dynamicSearchGroups.filter((group) => {
          ...
          if (group.id === pdfSpecialGroup) return false; // Exclude PDF → Excel from dropdown
    ...
    // Ref to track the last active group that wasn't PDF mode
    const lastActiveGroupRef = useRef<SearchGroup | undefined>(undefined);

    // Initial population of the ref
    useEffect(() => {
      if (!lastActiveGroupRef.current) {
        // Try to find the currently selected group in the visible groups, or fallback to the first one
        const current = orderedVisibleGroups.find(g => g.id === selectedGroup);
        if (current && current.id !== pdfSpecialGroup) {
          lastActiveGroupRef.current = current;
        } else {
          // Fallback defaults
          lastActiveGroupRef.current = orderedVisibleGroups.find(g => g.id === 'libeller')
            || orderedVisibleGroups.find(g => g.id === 'web')
            || orderedVisibleGroups[0];
        }
      }
    }, [orderedVisibleGroups, selectedGroup, pdfSpecialGroup]);

    // Update the ref whenever the selected group changes and is NOT the PDF group
    useEffect(() => {
      const current = orderedVisibleGroups.find(g => g.id === selectedGroup);
      if (current && current.id !== pdfSpecialGroup) {
        lastActiveGroupRef.current = current;
      }
    }, [selectedGroup, orderedVisibleGroups, pdfSpecialGroup]);
    ...
    const handleTogglePdfMode = useCallback(() => {
      if (isPdfMode) {
        const fallback =
          dynamicSearchGroups.find((group) => group.id === fallbackGroup) ??
          dynamicSearchGroups.find((group) => group.id !== pdfSpecialGroup);
        if (fallback) {
        ...
        const pdfGroup = dynamicSearchGroups.find((group) => group.id === pdfSpecialGroup);
        if (pdfGroup) {
          onGroupSelect(pdfGroup);
        }
      }
    }, [isPdfMode, onGroupSelect, dynamicSearchGroups, session, fallbackGroup, pdfSpecialGroup]);
    ...
    const handleOpenChange = useCallback(
      ...
      [isPdfMode, onGroupSelect, dynamicSearchGroups, fallbackGroup, pdfSpecialGroup],
    );
        ...
        <div className="flex items-center bg-background border border-accent/50 rounded-lg !gap-1 !py-1 !px-0.75 h-8">
            ...
            <Drawer open={open} onOpenChange={handleOpenChange}>
              <Tooltip>
                  ...
                  <DrawerTrigger asChild>
                    ...
                    >
                      ...
                      {isPdfMode && (
                        <>
                          <HugeiconsIcon
                            icon={lastActiveGroupRef.current?.icon || selectedGroupData?.icon || GlobalSearchIcon}
                            size={30}
                            color="currentColor"
                            strokeWidth={2}
                          />
                ...
                <TooltipContent side="bottom" className="max-w-[220px] p-2">
                  {isPdfMode ? (
                    <p className="text-xs">Quitter le mode PDF → Excel</p>
                  ) : selectedGroupData ? (
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                <HugeiconsIcon icon={File02Icon} size={30} color="currentColor" strokeWidth={2} />
              </Button>
            </TooltipTrigger>
            <TooltipContent side="bottom" className="max-w-[220px] p-2">
              <div className="space-y-1.5">
                ...
                <p className="text-[11px] leading-snug text-secondary">
                  Convertissez vos PDF en tableaux structurés prêts à être exportés en .xlsx
                </p>
                {!isProUser && (
                  <div className="pt-1 border-t border-border/50">
                    <a
                    ...
                    >
                      <HugeiconsIcon icon={Crown02Icon} size={14} strokeWidth={2} className="flex-shrink-0 text-secondary group-hover:scale-110 transition-transform" />
                      <span className="font-semibold text-[11px] text-secondary group-hover:underline flex items-center gap-0.5">
                        Débloquez les exports illimités avec Pro
                        <ArrowUpRight className="size-3 opacity-0 group-hover:opacity-100 transition-all group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
                      </span>
                    </a>
                  </div>
    ...
    if (isRecording && speechRecognitionRef.current) {
      ...
      try {
        if (typeof window === 'undefined') {
          toast.error('Voice recognition is only available in the browser.');
          return;
        }
