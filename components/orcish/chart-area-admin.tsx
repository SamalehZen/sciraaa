...
const chartConfig = {
  ...
  desktop: { label: "User", color: "var(--primary)" },
  mobile: { label: "Assistant", color: "var(--primary)" },
} satisfies ChartConfig;

export function ChartAreaAdmin() {
  ...
  const period = timeRange === "7d" ? "7d" : timeRange === "30d" ? "30d" : "30d"; // map 90d -> 30d

  const { data } = useQuery({
    queryKey: ["admin-metrics-orcish", period],
    queryFn: async () => {
      const r = await fetch(`/api/admin/metrics?period=${period}`);
  ...
  const timeSeries: Array<{ date: string; desktop: number; mobile: number }> =
    (data?.graphs?.timeSeries || []).map((d: any) => ({ date: d.date, desktop: d.user || 0, mobile: d.assistant || 0 }));
